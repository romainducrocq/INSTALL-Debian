...
if [ -d "home/romain/.local/bin" ] ; then
    PATH="home/romain/.local/bin:$PATH"
fi
...
# set PATH so it includes opt bin mangas if it exists
if [ -d "/opt/bin/mangas" ] ; then
	PATH="/opt/bin/mangas:$PATH"
fi
